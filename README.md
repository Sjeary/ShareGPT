...
- 克隆后可以直接安装依赖并运行界面
- 需要发布可用的 Sender / Receiver 安装包时，先按 `build/bin/README.md` 准备对应第三方二进制
...
macOS 的 Sender 发行包只需要 `sing-box`，不需要 `frpc`。如果要让打出的 Sender 包可以直接启动代理服务，请先准备 macOS 版 `sing-box`：
...
- 带 `--required` 的发行打包脚本会检查所需二进制，避免打出启动代理时不可用的包
