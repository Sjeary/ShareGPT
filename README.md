...
- **应用内更新**：以 **GitHub Releases** 为更新源（参考 [cc-switch](https://github.com/farion1231/cc-switch)，**不经过任何自建服务器**）检查最新版本、下载安装、保留账号/聊天记录/网页登录态。
...
自动更新参考 [cc-switch](https://github.com/farion1231/cc-switch) 的做法，**以 GitHub Releases 为更新源，不经过任何自建服务器**：

1. 把版本号写进 `package.json`（如 `5.5.0`），打包出安装包（Windows `.exe` / macOS `.dmg`）。
2. 在你**自己的 GitHub 仓库**新建一个 Release，tag 用 `v<版本号>`（如 `v5.5.0`），把 `.exe` / `.dmg` 作为附件上传。
3. 客户端会请求 `https://api.github.com/repos/<你的仓库>/releases/latest`，对比 tag 版本号；有新版就在登录页/账户页提示，并可一键下载安装（保留本机资料）。

> 检查哪个仓库由 `package.json` 的 `homepage` / `repository` 决定——**fork 的人改成自己的仓库地址即可**，更新会走他们自己的 Release，不会回到原作者这里。也可用 GitHub Actions（如 electron-builder 的 `--publish`）自动构建并上传 Release。
