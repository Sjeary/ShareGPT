...
团队的 AI 流量统一从这里出网，成员无需各自配梯子。两种形态做的是同一件事（一个能上外网的本地梯子 + sing-box vmess 中继 + frp 内网穿透）：

- **Linux 服务器（推荐）**：纯 CLI 部署，详见 [docs/RECEIVER.md](docs/RECEIVER.md)。
- **有桌面的机器**：装「接收端」GUI 版（`npm run dist:win:receiver`），界面里填同样字段、点开启——本质是上面那套中继套了个 UI。

客户端在「网络 / 代理」里填这套出口的对外参数（公网服务器 / 端口 / UUID）即可对接。
