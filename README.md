...
| 网络                      | 按需使用自己的 HTTP / SOCKS5 代理 | 普通成员可查看只读配置、选择获授权的代理方式；各 AI 默认出口由管理员指定 |
...
- **网络 / 代理（基于 [sing-box](https://sing-box.sagernet.org/)）**：
  - 个人工作区可使用自己的 HTTP 或 SOCKS5 代理；团队成员自动同步管理员配置，连接字段只读，已获授权的代理方式仍可选择。
...
- **协作聊天**：支持房间消息和私聊、图片与文件、回复、转发、撤回、已读、表情回应及离线补同步；成员列表持续显示在线人数和总人数。只标出最早未读位置，读到底后再进入保持在最新，翻看历史时保留原位置。组合表情加载失败可重试。
...
- **按需开启功能**：新用户的日历、待办、笔记和专注入口默认关闭，可在“账户”设置中开启，已有用户的选择保留。
- **资料隔离与接续**：个人与不同团队账号分别保存日历、待办、笔记、专注及团队聊天资料。旧资料可在“账户 → 本机资料”确认归属后按类别接续，原件保留。
- **连接与读取恢复**：掉线时可从右上角重新登录；本机资料读取失败时提供重试，不把异常当成空白资料覆盖保存。
...
管理员维护出口参数、线路授权与各 AI 推荐线路。普通成员可查看只读连接配置，并选择获授权的基础代理方式；不能编辑连接字段。各 AI 已指定默认出口时按该设置使用线路，高级独立环境保留各自的绑定。节点配置会下发给获授权客户端，不能把只读界面当作隐藏节点凭据的安全边界。
