...
- AI 网页打开时，折叠导航提示通过独立 renderer 覆盖在网页上方，但直接复用普通页面现有的 shadcn Tooltip 组件、Tailwind 样式和主题 token，不再由主进程复制一套近似 UI。
- 鼠标离开导航图标进入 ChatGPT、Claude 或 Gemini 网页后，提示会立即消失；点击进入 AI 页面不会因残留焦点把提示重新打开，窗口缩放、最大化、全屏和左右侧栏切换也会重置旧提示。
