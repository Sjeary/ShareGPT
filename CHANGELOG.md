...
> **1.0.9 稳定性收口**：保留既有多 AI Environment 与本机网页登录资料，补齐网页双语流程，并修复更新版本、登录恢复、账号隔离和页面复用问题。

### 新增

- 既有多 AI Environment 继续保留；管理员默认可使用高级环境与全部启用线路，普通用户仍按服务器授权使用。
- ChatGPT、Claude 和 Gemini 网页新增双语侧栏：可翻译网页选区或网页正文供侧栏阅读（过长正文单次读取前 30,000 字符），不替换网页原文；可按账号开启仅在三家官方 AI 页面生效的自动划词翻译（默认关闭），Claude 外部页面保留右键显式翻译；支持共用 AI provider、翻译 API 和只能连接本机回环地址的离线翻译服务。
- 所有已登录用户都可预览输入译文、翻译并填入或翻译并发送；高级 AI 权限只控制多环境和托管线路。站点语言默认英文且可配置。只有检测到明显不符合目标语言的文字时才暂停发送并要求确认，该提示基于文字系统启发式判断，不承诺区分所有拉丁语系语言。

### 交互改进

- 聊天消息操作菜单会根据滚动区域剩余空间自动翻转和限高，不再被消息列表边缘裁切。
- AI 导航与其它导航继续共用原有 renderer shadcn Tooltip、淡入淡出与悬停规则，不创建第二套 DOM 或主进程原生提示；内嵌网页覆盖提示时维持原有行为。

### 隔离与稳定性

- 修复更新提示先显示错误 `6.0.0`、登录后又被服务器版本覆盖的问题；桌面更新的显示、下载和安装现在都只认 GitHub Latest Release tag，错误或滞后的 `latest.yml` 版本与安装包文件名会在下载或安装前被拒绝。
- ChatGPT、Claude 和 Gemini 在应用面板之间切换时复用原有网页视图、标签与会话；同一线路不再重复执行阻断式出口预检或清空标签状态，换线路和授权变化仍会重新校验。
- 旧服务器未配置 expected IP、国家或 ASN 时不再凭空判定线路失败；已配置的身份仍严格匹配，SOCKS 会话绑定、独立 DNS、双链路实际 IPv4 出口检查继续保留。
- 记住密码后，应用重启或覆盖升级会自动向协作服务器换取新的登录 token，并恢复上次确认账号的 Principal；主动退出会关闭自动登录，但保留密码供下次手动登录。
- 管理员默认拥有高级 AI 能力和服务器当前启用的全部线路；高级环境资格不再与线路目录同步状态耦合，旧服务端或临时同步失败不会把已保存环境隐藏成不存在。
- 关闭高级功能、撤销权限、退出账号或升级只会停止并 flush 运行中的网页环境，不会删除 AI Environment、Cookie、LocalStorage、IndexedDB 或其他持久化网页数据；重新授权并开启后仍使用原分区。
- 修复并固化协作服务兼容性：升级服务器后，已发布的 1.0.x 客户端仍可登录、读取原有 `sender` / `update` 配置、连接实时消息、聊天和同步历史；新版优先使用更安全的 WebSocket 鉴权，服务端继续接受旧式连接，不再因鉴权方式不匹配而循环重登。
