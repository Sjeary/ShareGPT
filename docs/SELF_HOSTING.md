...
> 本文说明 ShareGPT `1.0.10` 的服务端与管理端；该版本正在准备发布，保留现有 `1.0.x` 客户端兼容。生产部署请使用已公开的 Release/tag；发布前的候选仅用于隔离验证。不能用桌面版本号代替服务端版本核对。
>
...
上例保留已发布的 `v1.0.9`，不要假设 `v1.0.10` 已经存在。1.0.10 发布并完成验收后再选择对应 tag；发布前仅在隔离测试环境使用经过审核的完整 40 位提交 SHA。旧服务可能不提供下文的全部新管理功能。升级前先备份数据和翻译加密密钥；桌面客户端升级不会自动更新服务器。
...
打开 ShareGPT Admin，登录 `https://collab.example.com`，进入“团队统一代理”。填写：
...
在“AI 线路策略”中导入并启用团队维护的线路，为每条线路设置稳定 ID、名称及可选的预期出口信息。然后在“成员与权限”中分别勾选账号可使用的线路：

- 管理员始终可管理所有已启用线路；
- “高级 AI”只控制是否能创建多个环境，不自动授予线路；
- 普通成员不需要也不能编辑团队连接参数，只会收到自己已获授权的线路结果。

  1.0.10 普通成员仍可查看只读连接参数，在代理停止时选择已获授权的基础代理方式；这与下方为各 AI 指定的默认出口是两个层次。指定默认出口后，普通成员的内嵌 AI 页面使用该线路；已有高级独立环境继续使用各自的线路绑定。

在同一“AI 线路策略”页面顶部，分别设置 ChatGPT、Claude 和 Gemini 的默认出口。默认出口会按用户授权解析；某成员未获授权时，服务端只会返回其可用线路中的安全回落，不会下发被拒绝线路的配置。

保存团队网络或线路目录后，1.0.9/1.0.10 客户端会停止旧线路并重新拉取配置；恢复连接后需要重新开启代理。1.0.8 及更早客户端需要退出账号后重新登录。1.0.9 的普通成员主页不显示或不能操作代理方式开关时，可由管理员为三种 AI 明确指定默认线路，无需因此授予高级权限。
...
管理端“AI 使用统计”分别显示 ChatGPT、Claude 和 Gemini 在所选日期范围内确认发送成功的次数与成员排行。这里记录的是客户端网页确认成功的发送，不是按键或点击尝试。

嵌入网页不会向 ShareGPT 返回模型 token 或供应商账单，因此该页面不估算 token 与费用；托管翻译服务返回的 token 与估算费用仍在“翻译服务”页面单独查看。
...
2. 打开 ShareGPT Admin →“AI 线路策略”。
3. 粘贴包含 `proxies:` 列表的 YAML。
4. 点击“解析节点”。
5. 选择显示为受支持的节点，检查转换后的 sing-box 出站预览，并保存为具有稳定 ID 的线路。
6. 在“成员与权限”中为各账号勾选允许使用的线路。
7. 在“AI 线路策略”顶部为 ChatGPT、Claude、Gemini 分别选择默认出口。
...
2. 普通成员的内嵌 AI 跟随管理员指定的默认线路；1.0.10 主页仍显示只读配置和获授权的基础代理方式。不要把基础代理选中项当成各 AI 最终出口的证明。
...
- 1.0.10 不再常驻显示机场稳定性提示；仍应通过代理检测及实际访问验证节点是否适合目标服务，不能仅凭节点启用状态判断可用。
...
先准备隔离副本验证，再在维护窗口逐实例升级；保留原数据路径、用户权限、代理配置、服务器身份文件和翻译加密主密钥。不要重新初始化管理员或重建用户库。若多个实例共用源码，切换前应明确哪些实例会受到影响。

1.0.10 的权限收口、个人同步资料恢复和删除记录保护需要升级后端；仅升级客户端或管理员端不会生效。新版服务器仍接收旧 1.0.x 的登录、配置及聊天协议。先验证旧客户端登录、两条已授权线路和公共/私聊，再验证新版客户端的资料同步与重连。

回滚优先切换回已验证的代码，保留运行期间新增的数据；不要直接用旧备份覆盖升级后产生的聊天或资料。涉及删除记录或服务器身份证明时，还要确认回滚版本理解这些数据，避免恢复后出现删除内容重现或身份冲突。
