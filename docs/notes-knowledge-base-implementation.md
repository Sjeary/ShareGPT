...
interface IndexApi {
  ...
  graph(scope: { mode: 'global' } | { mode: 'local'; path: string; depth: number })
    : Promise<{ nodes: GraphNode[]; links: GraphLink[] }>
}
interface AiApi {
  complete(req: { mode: 'expand'|'continue'|'summary'|'polish'|'rewrite'|'title'|'translate'|'linkSuggest';
                  text: string; notePath?: string }): Promise<{ streamId: string }>
