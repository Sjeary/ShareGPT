...
| 5   | 明文远程接口与敏感配置          | 部分正确 | 默认地址为空、私网与特殊地址拒绝、导出脱敏正确；公共远程 HTTP 是明确产品能力，保留明文风险警告后允许，不采纳“一律 HTTPS”。 |
...
| A 远程 HTTP 契约不一致                                           | 正确                 | `allowRemoteHttp` 仅由 Notes AI 和远程 translation 显式传入；offline 仍严格回环。HTTP/HTTPS 私网、metadata、special-use 及 DNS 任一危险结果均拒绝；连接地址 pinning 和 HTTPS SNI 保留。实现不跟随重定向，因此不新增可绕过的重定向路径。 |
| B principal 级隔离                                               | 正确                 | principal 绑定规范化 server origin + username 的 SHA-256；设置、普通/高级 partition、翻译状态和请求均隔离。旧数据只在保存的 server/username 能可靠归属时迁移，否则保留为 unowned 且不暴露，不删除用户数据。                             |
...
- principal 输入先规范化并验证：HTTP(S) server origin 去除路径与尾斜线，username 做 NFKC、trim 和小写，再生成不泄露原文的 SHA-256 标识。
- `advancedAi` 和 `translation` 按 principal 保存；A/B/A 切换各自恢复自己的设置。
- 普通 GPT/Claude/Gemini 与高级环境 partition 均包含 principalId，Cookie、LocalStorage、IndexedDB 不跨协作账号复用。
- principal 切换会取消主进程翻译、关闭旧 workspace、重置 renderer 翻译运行态并使旧 generation 失效。
...
- AI/远程翻译默认 endpoint 为空；公共 HTTP/HTTPS 可用，HTTP 在 UI 中保留明确明文传输警告。
- offline 翻译仅允许精确回环地址。
