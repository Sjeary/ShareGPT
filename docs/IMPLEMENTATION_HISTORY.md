...
> 更新日期：2026-09-13。本文面向维护者，记录版本边界、稳定实现原则和发布验收要求。面向用户的变化以 [`CHANGELOG.md`](../CHANGELOG.md) 为准，逐次发布步骤以 [`RELEASING.md`](RELEASING.md) 为准。

## 1. 当前状态

- 最新正式版本是 `v1.0.9`，发布于 2026-09-05；正式版本与资产以 GitHub Release 为准。
- `1.0.10` 正在准备发布。候选代码须通过完整 CI、真实 Electron 验收、安装升级及该版本分发策略检查，并进入 `main` 后才能发布。
- 桌面端使用 Electron `43.1.0`，开发和打包要求 Node.js `22.12.0` 或更高版本。依赖大版本升级与发布收尾分开处理。
...
- 本机业务资料按稳定账号/工作区隔离，旧资料通过明确确认复制接续；切换前保存草稿，失败时保持可恢复状态。
- 收口聊天顺序、唯一未读边界与切换前阅读位置；组合表情网络失败可恢复。
- 全局手动重登、密码更新恢复、环境删除后重建及清理重试。
- 开始/停止翻译使用不同按钮身份，覆盖停止操作跨越请求完成时不重复发起翻译的边界。
- 服务端加强账号初始化、管理员和聊天权限、个人资料恢复及删除记录兼容；旧 1.0.x 消费代码仍有协议回归。
- 用户说明保持“待发布”，正式日期在实际发布时固定。构建、签名方式和兼容下载名以发布指南为准。
...
- 默认公开路径使用 Developer ID、hardened runtime、公证与 staple。特定版本的明确分发例外见发布指南；版本和应用身份检查始终保留。
- 当前正式目标是 Apple Silicon `arm64`；若以后增加架构，需要独立的构建和升级验收。

### Windows

- 本地未签名 NSIS 只作候选验证，不能直接充当已签名公开产物。
- 默认公开路径使用有效 Authenticode 签名和时间戳，特定版本例外按发布指南执行；安装器、应用身份与更新元数据均需验证。
