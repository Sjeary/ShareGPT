...
> 更新日期：2026-08-20。本文是维护总账，记录已经落地的功能、实际验证证据、部署原则、构建经验和待办。
> 面向维护者；面向用户的版本变化以 [`CHANGELOG.md`](../CHANGELOG.md) 为准，逐次发布步骤以
> [`RELEASING.md`](RELEASING.md) 为准。

## 1. 当前状态

- Git 已发布代码基线：`main`，版本 `1.0.6`，基线提交 `de1e286`，对应 tag `v1.0.6`。
- 已发布版本仍为 `1.0.6`；`1.0.7` 只有在同一发布提交完成 Mac/Windows 构建验证、合入
  `main`、创建 tag 并上传完整 Release 资产后才算正式发布。
- `codex/chore-electron43-cross-platform` 分支承载 **1.0.7 发布改动**：macOS 强制保留原生平台、
  Electron 从 `31.7.7` 升到 `43.1.0`，并加强正式包完整性检查。根 `package.json` 已升为
  `1.0.7`；不得使用此前仍标记为 1.0.6 的候选产物，Mac/Windows 必须从最终发布提交重建。
...
### 3.4 1.0.7：macOS 原生平台与 Electron 43
...
- 锁文件已更新到修复当前生产依赖审计告警的 `ip-address 10.5.0`、`js-yaml 4.3.1/5.3.0`
  和 `ws 8.21.3`；Electron Builder 的开发构建链告警继续作为独立升级事项处理。
...
1. 在 Mac 重新运行全部校验并构建正式 1.0.7 DMG，记录大小、SHA-256 和签名状态。
2. Windows 拉取升版后的同一发布提交，使用 Node 22.12+ 安装依赖；重新运行测试、UI/隐私
   验证、NSIS 构建和 `verify:release-win`，记录正式 1.0.7 的体积与 SHA-256。
3. 单独解决 electron-builder 26 依赖扫描停滞；成功前不把版本升级写入正式构建流程。
4. 提交时只包含明确范围内文件；提交前复核 diff、凭据扫描、锁文件一致性和双平台证据。
5. 发布新 tag 和 Release；Windows 必须同时上传 NSIS、`latest.yml`、`.blockmap`，Mac 上传 DMG。
6. 用旧客户端验证兼容、用新客户端验证新增功能；确认后再逐实例增量更新协作后端。
