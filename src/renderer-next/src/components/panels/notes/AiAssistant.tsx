  ...
  return (
    <div className="space-y-2.5 p-3 text-sm">
      <p className="text-xs text-muted-foreground">
        配置 AI 接口 (OpenAI Responses / Codex 中转)。密钥仅存本机。
      </p>
      ...
      <div className="flex gap-2">
        ...
        <label className="block w-24 space-y-1">
          ...
          <select
            ...
            className="h-8 w-full rounded-md border border-border bg-background px-1 text-sm outline-none focus:border-primary/60"
          >
...
export function AiAssistant() {
  ...
  const writeTags = async () => {
    ...
    await useVaultStore
      .getState()
      .setFrontmatter(currentPath, { ...note.frontmatter, tags: merged })
  ...
  const suggestLinks = () => {
    ...
    const titles = Object.values(useVaultStore.getState().notesByPath)
      .filter(
        (n) => n.path !== currentPath && !n.path.endsWith('.canvas') && !n.path.endsWith('.base'),
      )
      .map((n) => n.title)
  ...
  const askRag = () => {
    ...
    start(
      ...
      { context },
      'text',
      '你是用户个人知识库的问答助手，用中文清晰、有条理地作答，可概括与归纳。',
    )
  }
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            disabled={running}
            onClick={() => start('tags', note.body, undefined, 'tags')}
            ...
            {!err && (
              <div className="mt-2 flex flex-wrap gap-1.5 border-t border-border/60 pt-2">
                {running ? (
                  ...
                  <>
                    {result && (
                      <Btn onClick={() => void navigator.clipboard.writeText(result)} icon={Copy}>
                        复制
                      </Btn>
                    )}
                    {result && kind === 'text' && (
                      <Btn onClick={() => void appendToNote(result)} icon={CornerDownLeft}>
                        插入文末
                      </Btn>
                    )}
                    {result && kind === 'tags' && (
                      <Btn onClick={() => void writeTags()} icon={Tag}>
                        写入标签
                      </Btn>
                    )}
                  </>
...
function Btn({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-1 rounded-md border border-border bg-card px-2 py-1 text-xs transition-colors hover:bg-accent"
    >
