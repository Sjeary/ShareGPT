...
// 简短展示面向用户的主要变化；详细变更以 GitHub Release notes 为准。
...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    highlights: [
      ...
      '掉线时可从右上角“重新登录”恢复；需要更新密码时可直接输入，网络失败后保留填写内容。',
      '修复 Windows 删除高级环境后无法重新创建的问题；普通成员可查看只读代理配置，并切换已获授权的代理方式。',
      '个人工作区和不同团队账号分别保存日历、待办、笔记与专注记录，团队聊天历史按账号单独保存；切换回来或重启后继续使用对应资料。',
      '在“账户 → 本机资料”中按类别接续旧资料，确认归属后复制到当前工作区，旧原件保留；旧聊天可按来源选择，已有资料不会被覆盖。',
      '新用户的日历、待办、笔记和专注入口默认关闭，可在账户设置中按需开启；已有设置保留。',
      '管理端增加三种 AI 的成功发送统计，并集中管理默认出口与线路授权。多设备删除记录和服务端数据保护需配套升级协作服务。',
    ],
  },
  {
    version: '1.0.9',
    date: '2026-09-05',
    highlights: [
      '首次启动可选择“连接团队”或“仅在本机使用”，从“账户”随时切换。个人与团队分别保留自己的设置和 AI 网页会话。',
      '翻译栏新增“阅读翻译”和“写给 AI”双流程：先看译文或发送预览，再决定复制或插入；支持选区/页面读取、草稿保护、停止翻译和拖动调宽。',
      '管理员可提供团队翻译服务、设置使用权限并查看用量，也可为不同 AI 推荐默认线路。成员直接使用下发配置，高级用户可为 AI 环境选择获授权线路。',
      '消息菜单和表情面板自动避让窗口边缘；切换会话可保留本次运行期间的文字草稿，发送失败不清空输入。阅读旧消息时不会被新消息拉回底部，查看引用后可返回原位置。',
      '聊天侧栏持续显示在线人数与成员总数，右上角消息提示可直接跳到对应会话；个人资料编辑适配窄窗口，保存失败保留内容供重试。',
      '升级协作服务器后可使用聊天历史与发送统计的备份恢复功能；网络诊断分别展示出口检测结果和配置状态。',
      '减少打开 AI 网页时重复的线路检查与初始化；标签横向滚动提示只在指针移入后显示。升级沿用原有数据目录，保留账号、配置与网页登录资料。',
    ],
  },
  {
    version: '1.0.8',
    date: '2026-08-26',
    highlights: [
      'ChatGPT、Gemini 和 Claude 支持多个独立网页登录环境，每个环境分别保存登录状态、网页数据和线路绑定。',
      '管理员可维护多条内置线路，并按用户开放高级 AI 环境及可用线路；线路检查失败时不会自动改走其它出口。',
      '新增翻译侧栏，可翻译手动输入、当前网页或选中文字，并支持 AI 兼容接口、LibreTranslate 和本机离线服务。',
      'Claude 新增按需网页入口；同时修复网页缩放、边界偏移、切换残留、不必要刷新和新标签加载中断。',
    ],
  },
  {
    version: '1.0.7',
