...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    highlights: [
      ...
      '团队管理员可提供加密保存的共享翻译服务、控制使用权限并查看用量；也可为不同 AI 推荐默认线路。首页团队代理参数仅管理员可改，高级用户仍可为 AI 环境选择获授权线路。',
      ...
      '升级协作服务器后，聊天历史和发送次数增加备份恢复保护；无法恢复时保留原件并报错。线路检测也会明确区分出口实测与配置检查，不再笼统显示“防泄漏通过”。',
      '减少打开 AI 网页时重复的线路检查与初始化；标签横向滚动提示只在指针移入后显示。升级沿用原有数据目录，保留账号、配置与网页登录资料。',
      '本版沿用历史分发方式，Windows 未正式签名，macOS 未经 Apple 公证，首次安装可能出现系统提示；请核对 GitHub Release 的下载来源和安装说明。',
    ],
  },
  {
    version: '1.0.8',
