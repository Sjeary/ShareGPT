...
test("跨设备只同步策略，不包含节点检测结果和本机清理记录", () => {
  ...
  assert.strictEqual(Object.hasOwn(payload, "localProfiles"), false);
  assert.strictEqual(Object.hasOwn(payload, "audit"), false);
  assert.strictEqual(payload.fingerprint.enabled, false);
...
test("环境应用会设置语言、时区、地理位置和 WebRTC 防泄漏策略", async () => {
  ...
  assert.strictEqual(
    commands.some(([name]) => name === "Emulation.setGeolocationOverride"),
    true,
  );
  assert.strictEqual(
    commands.some(([name]) => name === "Page.addScriptToEvaluateOnNewDocument"),
    true,
  );
});
